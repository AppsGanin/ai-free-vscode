...
export class QwenClient {
  ...
  async createChat(model = "qwen-max-latest") {
  ...
  async complete({
    ...
    if (!chatId) {
      chatId = await this.createChat(model);
    }
    ...
    if (!contentType.includes("event-stream")) {
      ...
      try {
        ...
        throw new Error(`Qwen: unexpected response: ${rawBody.slice(0, 200)}`);
      }
    ...
    for await (const chunk of res) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          if (this.debug)
            console.error("[qwen sse parse error] raw:", jsonStr.slice(0, 200));
          continue;
        }
